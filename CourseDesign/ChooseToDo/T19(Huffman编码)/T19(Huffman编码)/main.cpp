...
int main() {
	...
	Node* root = huffman(nodelist);    //建立Huffman树
	...
	writeCodeTxt();    //写入code.txt,这是一个存0/1的中间文件

	writeCodeDat();    //写入编码后的文章

	recodeFromDat();   //写入解码后的0/1字符到recodeFromDat.txt
